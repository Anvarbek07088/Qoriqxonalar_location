  ...
  useState(() => {
    ...
    if (userLocation.lat !== 0 && userLocation.lon !== 0) {
      const distances = parks.map((park) => {
        const distance = haversineDistance(userLocation.lat, userLocation.lon, park.lat, park.lon);
      ...
      <Container className="mt-4">
        <Row>
          <Col md={4}>
            ...
            <p>Hozirgi joylashuvingiz: {userLocation.lat ? `${userLocation.lat.toFixed(2)}, ${userLocation.lon.toFixed(2)}` : 'Yuklanmoqda...'}</p>

            <select className="form-select mb-4" onChange={(e) => setSelectedPark(parks.find(park => park.id === parseInt(e.target.value)))} >
            ...
            {selectedPark && (
              <Card>
                <Card.Body>
                  ...
                  <Card.Subtitle className="mb-2 text-muted">{selectedPark.region}</Card.Subtitle>
                  <Link >{selectedPark.description}</Link>
                </Card.Body>
              </Card>
            )}
            ...
            <div>
              ...
              <ul>
                {distances.map((dist, index) => (
                  <li key={index}>{dist.name}: {dist.distance} km</li>
                ))}
              </ul>
            </div>
          </Col>
          ...
          <Col md={8}>
            <MapContainer center={[userLocation.lat || 41.2995, userLocation.lon || 69.2401]} zoom={6} style={{ height: '500px', width: '100%' }}>
              <TileLayer
